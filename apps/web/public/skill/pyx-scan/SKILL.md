...
allowed-tools: WebFetch, Bash(curl -sf "https://scanner.pyxmate.com/*")
...
- **Security validation** — Both `owner` and `name` must match `^[a-zA-Z0-9._-]+$` (letters, digits, dots, hyphens, underscores only). Reject with an error if either contains characters outside this set. This prevents shell metacharacter injection (`$`, `` ` ``, `"`, `;`, `|`, `&`, etc.) in the curl fallback path.
...
If `WebFetch` fails (tool unavailable, network error), fall back to curl.
Only construct this command after `owner` and `name` have passed the validation in Step 1:

```bash
curl -sf "https://scanner.pyxmate.com/api/v1/check/{owner}/{name}"
```

**Note:** Because `owner` and `name` are validated to `[a-zA-Z0-9._-]+` in Step 1, no shell metacharacters can reach this command.
